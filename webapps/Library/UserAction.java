...
public class UserAction {
	...
	public static String getUserMailIdFromName(String name)
	{
		String mail="";
		Statement st=null;
		Connection con=null;
		try
		{
			...
			ResultSet rs=st.executeQuery("select mailId from userdetails where userName='"+name+"'");
			if(rs.next()){
				mail=rs.getString(1);
				con.close();
				st.close();
	...
	static int countMyRequestList(int userId) throws IOException 
	{
	...
	public static String rejectRequest(int groupId,int userId) {
		Connection con = null;
		PreparedStatement pst = null;
		Statement st=null;
		try {
			...
			pst.setInt(2, userId);
			pst.executeUpdate();
			return "Request Rejected Succcessfully";
			}
		 catch (SQLException e) {
			System.out.println(e);
		}
	...
	public static String acceptRequest(int groupId,int userId,String userName) {
		Connection con = null;
		PreparedStatement pst = null;
		Statement st=null;
		try {
			...
			pst.executeUpdate();
			st=con.createStatement();
			String insert="INSERT INTO groupuser(userId,groupId,memberName) VALUES('"+userId+"','"+groupId+"','"+userName+"')";
			st.executeUpdate(insert);
			return "Request Accepted Succcessfully";
			}
		 catch (SQLException e) {
			System.out.println(e);
		}
	...
	public static List<GroupRequest> viewMyRequestList(int userId) 
	{
		List<GroupRequest> showRequestListInfo=new ArrayList<GroupRequest>();
		Connection con = null;
		PreparedStatement st = null;
		try {
			con = db.getDbConnection();
			st = con.prepareStatement("SELECT grouprequest.groupId,grouprequest.userId,grouprequest.userName FROM grouprequest INNER JOIN groupinfo ON grouprequest.groupId=groupinfo.groupId AND groupinfo.ownerId=?");
			st.setInt(1, userId);
			ResultSet rs = st.executeQuery();
			while (rs.next()) {
				GroupRequest groupRequest=new GroupRequest();
				groupRequest.setGroupId(rs.getInt(1));
				groupRequest.setReqUserId(rs.getInt(2));
				groupRequest.setRequesterName(rs.getString(3));
				showRequestListInfo.add(groupRequest);
			}
			rs.close();
			st.close();
			con.close();
