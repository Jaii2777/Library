...
public class Http {
  ...
  public Object executeJSONRequest() throws Exception {
	  System.out.println("executeJSONRequest is called");
    JSONObject result = new JSONObject(executeRequestRaw());
	 System.out.println("yes");
	 if (result.getString("stat").equals("OK")) 
	 System.out.println("executeJSONRequest  result is"+result);
    if (! result.getString("stat").equals("OK")) {
  ...
  public String executeRequestRaw() throws Exception  {
	  try{
	    System.out.println("executeRequestRaw is called");
		Response response = executeHttpRequest();
		String k=response.body().string();
		System.out.println("executeRequestRaw  result is"+k);
		return k;
	  }catch(Exception e){
  ...
  public Object executeRequest() throws Exception {
	  System.out.println("executeRequest is called ");
  ...
  private Response executeRequest(Request request) throws Exception {
	...
	System.out.println(" inside the executeRequest response");
    while (true) {
      Response response = httpClient.newCall(request).execute();
	  System.out.println("inside execute request");
      if (response.code() != RATE_LIMIT_ERROR_CODE || backoffMs > MAX_BACKOFF_MS) {
		  System.out.println(" ans of response"+response);
        return response;
      }
	System.out.println("confirm ans of response"+response);
